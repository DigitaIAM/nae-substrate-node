...
```bash
./target/release/nae-node --dev
```

Purge the development chain's state:

```bash
./target/release/nae-node purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/nae-node -ldebug --dev
```
...
$ ./target/release/nae-node --dev --base-path ./my-chain-state/
...
(`cargo build --release && ./target/release/nae-node --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/nae-node --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/nae-node purge-chain --dev
